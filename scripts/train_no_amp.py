...
def process_rays_in_chunks(model, pts, view_pts, device, chunk_size=8192):
    ...
    for i in range(0, total_points, chunk_size):
        ...
        # REMOVED: AMP autocast context manager
...
def train(cfg: OmegaConf):
    ...
    # REMOVED: Mixed precision setup (GradScaler)
    # No longer using torch.amp.GradScaler()
    ...
    print(f"Starting training for {cfg.num_epochs} epochs (NO AMP - Full Precision)")
    ...
    for epoch in range(1, cfg.num_epochs + 1):
        for idx, ray_batch in enumerate(
            ...
            # REMOVED: Forward pass with mixed precision autocast
            # Standard forward pass in full precision (FP32)
            ...
            # MODIFIED: Standard backward pass without gradient scaling
            total_loss.backward()
            ...
            # MODIFIED: Standard optimizer step without scaler
            optimizer.step()
            scheduler.step()
